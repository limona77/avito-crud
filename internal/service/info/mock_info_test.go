...
func (m *MockInfoRepositoryRepository) GetReceivedTransactions(ctx context.Context, user string) ([]*model.Received, error) {
	args := m.Called(ctx, user)
	return args.Get(0).([]*model.Received), args.Error(1)
}

func (m *MockInfoRepositoryRepository) GetSentTransactions(ctx context.Context, user string) ([]*model.Sent, error) {
	args := m.Called(ctx, user)
